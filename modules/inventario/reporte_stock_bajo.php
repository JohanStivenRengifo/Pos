...
try {
    ...
    class StockReportPDF extends FPDF {
        function Header() {
            ...
            $this->SetTextColor(0);
            $this->Cell(0, 6, $empresa['nombre_empresa'], 0, 1, 'C');
            $this->Cell(0, 6, 'NIT: ' . $empresa['nit'], 0, 1, 'C');
            $this->Cell(0, 6, $empresa['direccion'], 0, 1, 'C');
            $this->Cell(0, 6, 'Fecha de generacion: ' . date('d/m/Y H:i'), 0, 1, 'C');
    ...
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(100, 7, 'Total de productos con stock critico', 1, 0, 'L');
    ...
    $pdf->Cell(100, 7, 'Inversion necesaria para reposicion', 1, 0, 'L');
